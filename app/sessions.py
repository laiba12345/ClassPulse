...
class SessionRegistry:
    ...
    def create(self, fixture_id: str, session_id: str | None = None) -> SessionRecord:
        ...
        runtime = ClassRuntime(lesson, self.provider_factory(), memory=self.memory_factory(), session_id=session_id)
        record = SessionRecord(session_id, fixture_id, runtime, datetime.now(timezone.utc).isoformat())
