...
async def _collect(name: str) -> dict:
    ...
    async for message in runtime.run(speed=100_000):
        if message["kind"] == "event":
            ...
            at = current_event["at"]
            timeline.append({"at": at, "event_type": current_event["type"], "score": message["data"]["score"], "ground_truth_confused": window["start"] <= at <= window["end"]})
...
def write_report(results: list[dict], output: Path = DEFAULT_REPORT) -> Path:
    ...
    for item in results:
        lines.append(f"| {item['title']} | {item['window_precision']:.3f} | {item['window_recall']:.3f} | {item['poll_prediction_accuracy']:.3f} ({item['polls']} polls) |")
    lines += [
        "", "## Aggregate", "", f"- Authored-window precision: **{precision:.3f}**", f"- Authored-window recall: **{recall:.3f}**",
        f"- Pre-poll majority-miss prediction: **{poll_correct}/{poll_total}**", f"- Confusion matrix: `{total}`", "",
