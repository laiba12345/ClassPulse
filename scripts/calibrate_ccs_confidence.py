"""Bucket CCS alert confidence against authored-window empirical precision."""
...
def write_report(calibration: dict, output: Path = DEFAULT_REPORT) -> Path:
    lines = [
        ...
        f"**Status: {calibration['status']}.** Confidence is an evidence-quality heuristic, not a validated probability of correctness.", "",
    ...
    for bucket in calibration["buckets"]:
        lines.append(f"| {bucket['range']} | {bucket['count']} | {bucket['average_confidence']:.3f} | {bucket['empirical_precision']:.3f} | {bucket['absolute_gap']:.3f} |")
    lines += [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check CCS confidence calibration on authored fixtures.")
